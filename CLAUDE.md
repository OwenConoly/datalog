...
- `Operational.v` — synchronous whole-program reference semantics (one positional node per rule, broadcast everywhere); soundness (`comp_steps_sound`) and completeness (`comp_step_complete`) are both proved. `OperationalWithIO.v` re-presents `comp_step` as an IO-labelled small-step (`step`, with `I_event`/`O_event`); its `produces`-based soundness/completeness theorems (`produces_sound`, `produces_complete`) are proved for the single-pool model (an `I_event` adds its fact straight to the global `known_facts`; input-step preservation of `meta_facts_ok`/`meta_facts_correct` reduces to `knows_incl known (f::known)`, obtained from `sane_allowed_inputs` + `knows_incl_of_submultiset`). `OperationalToDistributed.v` is the in-progress bridge to the per-node/distributed semantics; it is excluded from the build (it still references the pre-single-pool `node_state`/`sent_facts`/`waiting_facts` API).
