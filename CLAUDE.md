...
Build status: the core `src/datalog` theory (`AggregatingProgram`, `Blocks`, `Datalog`, `Distributed`, `ForwardingNode`, `Interpreter`, `NattifyRel`, `Node`, `Operational`, `OperationalWithIO`, `RelMap`) plus `src/util` compile end-to-end. `OperationalToDistributed.v` is currently broken (see below), so a full `dune build` is red on that module; build the individual `.vo` targets to check the rest. `src/datalog/dune` excludes four WIP modules from the theory (`modules :standard \ Local JSON QueryableToRunnable FancyNotations`): `QueryableToRunnable.v` and `src/atl/*.v` predate the core-API refactor (record facts, `prog_impl_fact`, `clause_R`, `node_prog`) and don't compile against the current `Datalog.v`/`Node.v`; `Local.v` (indexed single-node impl) has an Aborted `compiler_correct` and a standing type error; `JSON.v` needs coq-json.
