...
Build status (July 2026): `src/atl/*.v` (the ATL→Datalog compiler), `src/datalog/QueryableToRunnable.v`, and `src/datalog/OperationalToDistributed.v` predate the core-API refactor (record facts, `prog_impl_fact`, `clause_R`, `node_prog`) and do not compile against the current `Datalog.v`/`Node.v`, so the full `make` cannot complete; work from `make datalog`. The `datalog` target's file list omits `Distributed.v` — build it with `make -f Makefile.coq src/datalog/Distributed.vo`. Stale `.vo` files may exist for the broken files; do not trust them.
...
- `Operational.v` — synchronous whole-program reference semantics (one positional node per rule, broadcast everywhere); soundness (`comp_steps_sound`) and completeness (`comp_step_complete`) are both proved. `OperationalToDistributed.v` was the bridge from it to the per-node semantics but is stale.
