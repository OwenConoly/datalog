...
Build status: default `dune build` is green. The core `src/datalog` theory (`AggregatingProgram`, `Blocks`, `Datalog`, `Distributed`, `FixpointSemantics`, `ForwardingNode`, `Interpreter`, `NattifyRel`, `Node`, `Operational`, `RelMap`, `TypedDatalog`) plus `src/util` compile end-to-end. `src/datalog/dune` excludes five WIP modules from the theory (`modules :standard \ Local JSON QueryableToRunnable OperationalToDistributed FancyNotations`): `QueryableToRunnable.v` and `src/atl/*.v` predate the core-API refactor (record facts, `prog_impl_fact`, `clause_R`, `node_prog`) and don't compile against the current `Datalog.v`/`Node.v`; `Local.v` (indexed single-node impl) has an Aborted `compiler_correct` and a standing type error; `OperationalToDistributed.v` still references the pre-single-pool `node_state`/`sent_facts`/`waiting_facts` API; `JSON.v` needs coq-json.
...
- `Operational.v` — synchronous whole-program reference semantics (one positional node per rule, broadcast everywhere); soundness (`comp_steps_sound`) and completeness (`comp_step_complete`) are both proved, and `prog_impl_iff_comp_step` packages them as the biconditional between `prog_impl` over a finite input set (`knows_datalog_fact inputs`) and `comp_step^*` reachability from the corresponding `start` state (`known_facts := inputs`, empty per-node sent-lists). `OperationalToDistributed.v` is the in-progress bridge to the per-node/distributed semantics; it is excluded from the build (it still references the pre-single-pool `node_state`/`sent_facts`/`waiting_facts` API).
...
- `RelMap.v` holds the proven cross-type relation-renaming machinery (`map_fact`, `map_program`, main theorems `interp_map_iff` and `interp_map_iff_inj`), parameterized by an arbitrary `f : rel1 -> rel2`. `Blocks.v` requires it and is the staging/composition layer (PHOAS `LetIn`/`Block` trees over rule fragments): inlining to one flat program (`flatten`, `flatten_correct'`); `Datalog.v`'s `staged_program_iff` composes conclusion-disjoint programs. `AggregatingProgram.v` compiles a small PHOAS aggregation DSL onto blocks (proven). `NattifyRel.v` renames relations to `nat` via a per-program table (`nattify_rel_prog`, proven `nattify_rel_correct`), built on `RelMap.v`. `Local.v` is the indexed single-node implementation (per-relation `idx_struct` key/value column masks, incrementally maintained aggregates); its `compiler_correct` is stated but Aborted.
