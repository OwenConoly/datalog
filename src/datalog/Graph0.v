...
Section __.
  ...
  Section graph.
    ...
    Context {node_state : Type} {node_states : map.map node_id (node_state * list (IO_event))}.
    ...
    | gstep_run gs n np ns t ns' outs :
      ...
      graph_step gs (O_event (filter (output_visible n) outs))
                 {| g_nodes := map.put gs.(g_nodes) n (ns', O_event outs :: t);
  ...
  Section graph.
    ...
    Print can_output. Print graph_step.
    From coqutil Require Import Tactics.fwd.
    Lemma lift_can_weakening gs1 gs2 t1 t2 :
      Forall4_map (fun _ np np '(ns1, t1) '(ns2, t2) =>
                     forall o,
                       can_output (node_step np) ns1 t1 o ->
                       can_output (node_step np) ns2 t2 o)
        p p gs1.(g_nodes) gs2.(g_nodes) ->
      forall o,
        can_output (graph_step p node_step) gs1 t1 o ->
        can_output (graph_step p node_step) gs2 t2 o.
    Proof.
      intros H o Ho. cbv [can_output] in Ho. fwd.
      induction Hop0.
      - cbv [can_output]. exists nil, gs2. split; [constructor|].
        split; auto. asadmit.
      -
      cbv [can_output].
    ...
    Proof.
      ...
      - cbn in Hinp'. destruct e as [m | outs]; [discriminate|]. cbn in Hinp'.
        ...
        + (* Deeper case: o is in t'_rest.  Use eventually_step_cps to advance
          ...
          * (* Demon non-empty: use graph_will_output_star_preserved to advance
               will_output along the demon's play.  But we need an initial
               will_output to advance — derived inductively (or by drive_node_must
               on the producing step of t'_rest). *)
            (* TODO: structurally, we want: derive graph will_output at gs (for
               the angel's path Hstep+Hstar'_rest producing o), then apply
               graph_will_output_star_preserved along Hstar_demon. *)
            admit.
    Admitted.
  End graph.
