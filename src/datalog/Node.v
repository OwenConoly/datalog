...
Section __.
  ...
  (* ---- The le-simulation, mirroring Graph.v. ----
     ...
     fed through [node_step]) for [le], and by [inputs_of gns_trace ++ gns_queue]
     (those plus messages delivered-but-not-yet-consumed) for [le_weak].  At a
     standalone node there is no external delivery buffer: every arrival is an
     [I_event] (appended to [waiting_facts]), and the dequeue that later moves a
     fact into [known_facts] is an internal [O_event], invisible to [inputs_of].
     So the node's whole input trace is [known_facts ++ waiting_facts ~ inputs_of t],
     and BOTH orderings compare that multiset -- they differ only in the relation:
     ...
     Putting [nle] on [known_facts ++ waiting_facts] (not just [known_facts]) is
     what makes the [le_node_output] analogue hold: that lemma is the direct form
     of [might_implies_will_equiv'], whose domination hypothesis is exactly
     [incl_mod dfact_equiv dfact_consistent (inputs_of t1) (inputs_of t2)], and
     it drives [s2] (forcing its own dequeues/deduces), so it must constrain what
     [s2] has *received*, not what it has already dequeued. *)

  Definition nle_weak (s1 s2 : node_state) : Prop :=
    incl_mod_weak dfact_equiv
      (s1.(known_facts) ++ s1.(waiting_facts))
      (s2.(known_facts) ++ s2.(waiting_facts)).

  Definition nle (s1 s2 : node_state) : Prop :=
    incl_mod dfact_equiv dfact_consistent
      (s1.(known_facts) ++ s1.(waiting_facts))
      (s2.(known_facts) ++ s2.(waiting_facts)).
  ...
  (* Analogue of Graph.v's [node_will_match']: after [s1] takes one output step,
     ...
     At the single node this is expected to be near-trivial -- there is no
     self-enqueue, so an [O_event] step (dequeue or deduce) leaves the multiset
     [known_facts ++ waiting_facts] unchanged, hence preserves [nle_weak].  The
     real dequeue-forcing that Graph.v hides inside its broadcast catch-up lives
     instead in the [le_weak_to_le] analogue.  [nle s1 s2] is kept for a faithful
     mirror of Graph.v and may turn out to be unnecessary here. *)
