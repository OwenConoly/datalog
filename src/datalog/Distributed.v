...
Section Distributed.
  ...
  Ltac map_func := cbv [mf_label] in *; Datalog.Util.Tactics.map_func.
  ...
  Context (Hsender : Forall_map (fun n p => sends_concl_rels R_senders (node_source n) p) graph_prog).
  ...
  Local Notation nstep := (fun n => node_step R_senders (prog_at n) (node_source n)).
  ...
  Lemma node_outputs_well_formed (k : node_id) (p : list rule)
      (forward : source -> destn -> dfact -> bool)
      (forward_rel : forall s d f g,
          dfact_rel f = dfact_rel g -> forward s d f = forward s d g) :
    sends_concl_rels R_senders (node_source k) p ->
    outputs_well_formed (node_step R_senders p (node_source k))
  ...
  Proof.
    ...
    assert (Hsend : forall s0 f, new_facts R_senders p name s0 f -> In name (R_senders (dfact_rel f))).
    ...
    assert (Hsrc : forall R a src num,
               In (meta_dfact R a src num) s.(sent_facts) -> src = name)
      by (eapply sent_source_correct; exact Hstar).
    assert (Hcnt : forall R a num,
               In (meta_dfact R a name num) s.(sent_facts) ->
    ...
    - intros (R & mf_args) Hclaim. cbv [claim_output consistent_output] in Hclaim |- *.
      ...
      + intros x (nfa & -> & _).
        rewrite (forward_rel name (node_destn dest) (normal_dfact R nfa)
                   (meta_dfact R mf_args name cnt) eq_refl).
        exact Hfwd.
